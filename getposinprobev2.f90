...
do j = 1, ncyc
	...
	tWheel = t0 + abs(colposvalve(3)/speed)

	colposwheel(1) = direction(1) * tWheel
	colposwheel(2) = direction(2) * tWheel
	colposwheel(3) = direction(3) * tWheel

	!print *, 'colposwheel is', colposwheel(3)
	...
	do t = 1, NumberOfTimepoints
		...
		if (Tdelay(t) .lt. tWheel) then
			...
			!print *, 'initial colposprobe(3) is', colposprobe(3)

			!print *, 'pos z is', colposprobe(3)

		else

			velocity(1) = scatteredDirection(1)*scatteredSpeed
			velocity(2) = scatteredDirection(2)*scatteredSpeed
			velocity(3) = scatteredDirection(3)*scatteredSpeed

			!print *, 'scattered velcoity is', velocity(3)

			colposprobe(1) = colposwheel(1)+(velocity(1)*(Tdelay(t)-tWheel))
			colposprobe(2) = colposwheel(2)+(velocity(2)*(Tdelay(t)-tWheel))
			colposprobe(3) = colposwheel(3)+(velocity(3)*(Tdelay(t)-tWheel))

			!print *, 'scattered colposprobe(3) is', colposprobe(3)

		end if

		!tFrontSheet = t0 + ((colposvalve(3)-(LScentre+LSwidth)) / (abs(direction(3))*speed))
		!tBackSheet = t0 + ((colposvalve(3)-(LScentre-LSwidth)) / (abs(direction(3))*speed))

		!print *, 'print *, colposprobe(3) is', colposprobe(3)
		!print *, 'LScentre - (LSwidth/2) is', LScentre - (LSwidth/2)
		!print *, 'Tdelay(t) is', Tdelay(t)
		! *, 'tWheel is', tWheel
		!print *, 'Tdelay(t) - tWheel is', Tdelay(t)-tWheel
		!if molecule has reached the laser sheet or has exited the other side of the sheet then it is written to file
		...
		if ((zDirection .ge. bottomEdge) .and. (zDirection .le. topEdge)) then
			...
			if (zDirection .gt. 0) then
				...
				if ((yDirection .gt. -1*LSHeight) .and. (yDirection .le. LSheight)) then
					...
					colposprobePX(3) = ceiling(abs(((colposprobe(3) - 0.036)/pxmmratio)))
